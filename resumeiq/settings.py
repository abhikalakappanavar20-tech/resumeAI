...
SECRET_KEY = os.environ.get('DJANGO_SECRET_KEY', 'django-insecure-resumeiq-dev-key-change-in-production')
...
if IS_VERCEL:
    ...
    if vercel_url:
        ALLOWED_HOSTS.append(vercel_url.replace('https://', '').split('/')[0])
    if branch_url:
        ALLOWED_HOSTS.append(branch_url)
...
if DATABASE_URL:
    if '[YOUR-PASSWORD]' in DATABASE_URL or '[YOUR-' in DATABASE_URL:
        DATABASE_URL = ''
    else:
        try:
...
MEDIA_URL = '/media/'
MEDIA_ROOT = '/tmp/media/' if IS_VERCEL else str(BASE_DIR / 'media')
